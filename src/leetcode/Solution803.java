 ...
 *          一块砖直接连接到网格的顶部，或者 至少有一块相邻（4 个方向之一）砖块 稳定 不会掉落时
 ...
 *      所有 (xi, yi) 互不相同
 * @author: Deepcola
 * @time: 2021/1/16 14:58
 */
public class Solution803 {

    /**
     * 并查集:
     *      顺序敲碎砖头相当于逆序补上砖头后每次多出来的稳定砖头
     */
    private int row;
    private int col;
    ...
    public int[] hitBricks(int[][] grid, int[][] hits) {
        ...
        // 其余网格，如果是砖块: 向上、向左看一下，如果也是砖块，在并查集中进行合并 (相连的砖块也稳定 -> 只用考虑左上即可确定所有联通分量)
        for (int i = 1; i < row; i++) {
            for (int j = 0; j < col; j++) {
                if (copy[i][j] == 1) {
                    // 上方有砖块
        ...
        // 第 3 步：按照 hits 的逆序，在 copy 中补回砖块，把每一次因为补回砖块而与屋顶相连的砖块的增量记录到 result 数组中
